...
dat$sample_num <- factor(dat$sample_num,
                         levels =  c(bb%>%filter(group=='Control')%>%.$sample_num%>%as.numeric%>%sort, #转化为数字，并排序
...
png("Fig1H.clusters Barplots-sample_and_cluster-Anno.chi1.png", width = 12, height = 6, units = "in", res = 400) #只能是png
ggplot(data=dat_plot@meta.data, aes(x=sample_num, fill=cluster)) +
  ...
  theme(
    ...
    axis.title.x = element_text(size=14, family="Times New Roman"),  # X轴标题字体
    axis.title.y = element_text(size=14, family="Times New Roman"),  # Y轴标题字体
    legend.text = element_text(size=14, family="Times New Roman"),  # 图例文字字体
    legend.title = element_text(size=16, family="Times New Roman"), # 图例标题字体
    plot.title = element_text(size=16, family="Times New Roman", face="bold")  # 图表标题字体
  )
